...
from ..inference.sampling import get_alphas_sigmas, sample, sample_discrete_euler, truncated_logistic_normal_rescaled, sample_timesteps_logsnr
...
from .utils import create_optimizer_from_config, create_scheduler_from_config, mask_from_frac_lengths, generate_mask, generate_channel_mask, log_audio, log_image, log_metric, log_point_cloud
import os
...
class DiffusionInfillDemoCallback(Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):        
        ...
        try:
            ...
            fakes = fakes.to(torch.float32).div(torch.max(torch.abs(fakes))).mul(32767).to(torch.int16).cpu()
            torchaudio.save(filename, fakes, self.sample_rate)
            log_audio(trainer.logger, f'demo_ssl_{trainer.global_step:08}', filename, self.sample_rate)
            log_image(trainer.logger, f'demo_melspec_left', audio_spectrogram_image(fakes))
...
class DiffusionCondTrainingWrapper(L.LightningModule):
    ...
    def __init__(
        ...
        self.video_dropout_prob = video_dropout_prob

        self.rng = torch.quasirandom.SobolEngine(1, scramble=True)

        self.timestep_sampler = timestep_sampler
        if self.timestep_sampler == "log_snr":
            self.mean_logsnr = -1.2
            self.std_logsnr = 2.0
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.pre_encoded:
            if reals.shape[1] != 64:
                ...
                waveform = torch.stack([item['waveform'] for item in metadata],dim=0)
                diffusion_input = waveform
            loss_info["audio_reals"] = diffusion_input
        else:
            diffusion_input = reals
        #print(diffusion_input,flush=True)
        p.tick("setup")

        conditioning = self.diffusion.conditioner(metadata, self.device)            

        # video_exist = torch.stack([item['video_exist'] for item in metadata],dim=0)
        # if isinstance(conditioning['metaclip_features'], list):
        #     empty_clip_feat = self.diffusion.model.model.empty_clip_feat.to(conditioning['metaclip_features'][0].dtype)
        #     conditioning['metaclip_features'][0][~video_exist] = empty_clip_feat
        #     if 'sync_features' in conditioning.keys():
        #         empty_sync_feat = self.diffusion.model.model.empty_sync_feat.to(conditioning['sync_features'][0].dtype)
        #         conditioning['sync_features'][0][~video_exist] = empty_sync_feat
        # else:
        ...
        elif self.max_mask_segments == -1:
            source_latent = torch.stack([item['source_latent'] for item in metadata],dim=0)
            conditioning['inpaint_masked_input'] = source_latent
        ...
        output = self.diffusion(noised_inputs, t, cond=conditioning, cfg_dropout_prob = self.cfg_dropout_prob, video_dropout_prob=self.video_dropout_prob, **extra_args)
        p.tick("diffusion")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.max_mask_segments > 0:
            ...
            conditioning['inpaint_mask'] = [mask]
            conditioning['inpaint_masked_input'] = masked_input
        # elif 'source_latent' in conditioning.keys():
        #     conditioning['inpaint_masked_input'] = conditioning['source_latent']
        for validation_timestep in self.validation_timesteps: 
            ...
            alphas = alphas[:, None, None]
            sigmas = sigmas[:, None, None]
            #print("diffusion_input",diffusion_input.dtype)
            noise = torch.randn_like(diffusion_input)
            noised_inputs = diffusion_input * alphas + noise * sigmas
            ...
            with torch.no_grad():
                output = self.diffusion(noised_inputs, t, cond=conditioning, cfg_dropout_prob = 0.0, video_dropout_prob=0.0)
    ...
    def on_validation_epoch_end(self):
        ...
        # Gather losses across all GPUs
        val_loss = self.all_gather(val_loss).mean().item()
        log_dict['val/avg_loss'] = val_loss
    ...
    def predict_step(self, batch, batch_idx):
        ...
        batch_size, length = reals.shape[0], 194
        conditioning = self.diffusion.conditioner(metadata, self.device)
        ...
        # if self.max_mask_segments > 0:
        #     bsz, dim, seq_len = reals.shape
        #     mask_start = torch.randint(0, seq_len-43+1,(1,)).item()
        #     inpaint_mask = torch.ones((bsz, seq_len), dtype=torch.bool, device=self.device)
        #     inpaint_mask[:, mask_start:mask_start+43] = 0
        #     inpaint_mask = inpaint_mask.unsqueeze(1).expand(-1, dim, -1)
        #     masked_input = reals * inpaint_mask
        #     conditioning['inpaint_masked_input'] = masked_input
        if self.max_mask_segments > 0:
            # 读取音频文件
            # import ipdb
            # ipdb.set_trace()
            # audio_path = 'interactive/iter1/_8JGpCc5RI8_000160_stage2.mp4'
            # reals, sample_rate = torchaudio.load(audio_path)
            # reals = reals.unsqueeze(0).to(self.device)
            # reals = self.diffusion.pretransform.encode(reals)
            bsz, dim, seq_len = reals.shape
            inpaint_mask = torch.ones((bsz, seq_len), dtype=torch.bool, device=self.device)
            inpaint_mask[:, 100:140] = 0
            inpaint_mask = inpaint_mask.unsqueeze(1).expand(-1, dim, -1)
            masked_input = reals * inpaint_mask
            conditioning['inpaint_masked_input'] = masked_input
        elif self.max_mask_segments ==-1:
        ...
        if self.diffusion_objective == "v":
            ...
            #     fakes = sample_discrete_euler(model, noise, 24, **cond_inputs, cfg_scale=5, batch_cfg=True)
            #     if self.max_mask_segments > 0:
            #         masked_input = torch.zeros_like(fakes, dtype=fakes.dtype, device=self.device)
            #         masked_input[:,:,:100] = fakes[:,:,-100:]
            #         conditioning['inpaint_masked_input'] = masked_input
            ...
            #     else:
            #         # 后续迭代，只保存最后94长度的音频
            #         fakes_list.append(fakes[:, :, -94:])

            # # 组合所有音频片段
            # fakes = torch.cat(fakes_list, dim=2)
            end_time = time.time()
        ...
        if self.diffusion.pretransform is not None:
            ...
            if self.max_mask_segments > 0:
                masked_input = self.diffusion.pretransform.decode(masked_input)
            elif self.max_mask_segments == -1:
                masked_input = self.diffusion.pretransform.decode(source_latent)


        audios = fakes.to(torch.float32).div(torch.max(torch.abs(fakes))).clamp(-1, 1).mul(32767).to(torch.int16).cpu()
        if self.max_mask_segments > 0:
            masked_audios = masked_input.to(torch.float32).div(torch.max(torch.abs(masked_input))).clamp(-1, 1).mul(32767).to(torch.int16).cpu()
            return (masked_audios, audios)
        elif self.max_mask_segments==-1:
            masked_audios = masked_input.to(torch.float32).div(torch.max(torch.abs(masked_input))).clamp(-1, 1).mul(32767).to(torch.int16).cpu()
            return (masked_audios, audios)
        return audios
    
    def predict_step_inpaint(self, batch, batch_idx):
        reals, metadata = batch
        # import ipdb
        # ipdb.set_trace()
        ids = [item['id'] for item in metadata]
    ...
    def export_model(self, path, use_safetensors=False):
        try:
            ...
            print(f"failed to export model to: {path}. {e} occured")
